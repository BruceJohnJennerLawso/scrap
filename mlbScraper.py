...
def scrapeTeamData(teamId, debugInfo, seasonId, inProgressSeason, leagueId):
	...
	page = requests.get('http://www.baseball-reference.com/teams/%s/%s-schedule-scores.shtml' % (teamId, seasonId))
	tree = html.fromstring(page.content)

	gamesLists = []
	
	teamInfo = tree.xpath('//h1/text()')
	if(debugInfo):
	...
	page = requests.get('http://www.hockey-reference.com/teams/%s/%s-roster.shtml' % (teamId, seasonId))
	tree = html.fromstring(page.content)	
...
if(__name__ == "__main__"):
	...
	scrapeTeamData('TOR', True, '2016', False, 'mlb')		
