...
def generateHistogram(xlabel, ylabel, title, values, output_path, output_directory, output_filename, subplot_col, subplot_row, subplot_no, minShow='foo', maxShow='bar', binCount=39):
	...
	minShow, maxShow, interval = getGraphBounds(values, 5)  
...
def isPowerOf(value, powerOf):
	initialValue = value
	def isNotWithin(value, lower, upper):
		if((value <= upper)and(value >= lower)):
			return False
		else:
			return True
	while(isNotWithin((value/10.0), 0.1, 1.0)):
		if( (value/10.0) > 1.0):
			value /= 10.0
		elif((value/10.0) < 0.1):
			value *= 10.0
...
if(__name__ == "__mainer__"):
	values_to_check = [100, 50, 20, 10,5,2,0.5, 0.1, 0.05, 0.01, 0.005, 0.001]
...
def getGraphBounds(values, snapToNearest=10, paddingLayers=0, debugInfo=False):
		...
		if((viewFilledPercentage != -1.0)and(viewFilledPercentage < cutoff)):
			...
			if(snapToNearest > 2.0):
				if((isPowerOf(snapToNearest, 10)) or (isPowerOf(snapToNearest, 2))):
					snapToNearest *= 0.5
				elif(isPowerOf(snapToNearest, 5)):	
					snapToNearest *= 0.4
			elif(snapToNearest == 2.0):
				snapToNearest *= 0.25
				## the black sheep of the group
				## nothing really *wrong* here, all of the values that the
				## algorithm targets are only really targeted because they look
				## good on paper
			elif(snapToNearest < 2.0):
				## values less than 1.0, so we go 0.5, 0.1, 0.05, 0.01,0.005
				if(isPowerOf(snapToNearest, 10)):
					snapToNearest *= 0.5
	...
	return (lower, upper, float(snapToNearest))
...
def plotScatterplot(xlabel, ylabel, title, x_values, y_values, output_path, output_directory, output_filename, minShow='foo', maxShow='bar', binCount=39):
	...
	if(minShow == 'foo'):
		minShow, maxShow, xInterval = getGraphBounds(x_values)
	...
	##major_ticks_x = np.arange(minShow, maxShow, xInterval)                                              
	##minor_ticks_x = np.arange(minShow, maxShow, xInterval*10.0)    

	##major_ticks_y = np.arange(minY, maxY, yInterval)                                              
	##minor_ticks_y = np.arange(minY, maxY, yInterval*10.0) 	                                           
	...
	## this just refuses to cooperate, so Im leaving it turned off for the moment	
	ax.grid(which='minor', alpha=0.2)                                                
	ax.grid(which='major', alpha=0.5)
	
	plt.plot(x_values, y_model, "g")
	## plot the base model without any modifiers
	plt.plot(x_values, y_values, 'bs', markersize = 6)
